...
/// Export all model weights in tch `VarStore` format.
///
...
pub fn export_checkpoint(
    ...
    let weights_path = out_dir.join(format!("{name}.pt"));
...
Generated by tak-export. Requires: torch
...
mod tests {
    ...
    fn checkpoint_export_creates_files() {
        ...
        assert!(dir.join("student.pt").exists());
    ...
    fn onnx_script_generated() {
        ...
        let checkpoint = dir.join("model.pt");
        ...
        assert!(content.contains("model.pt"));
        assert!(content.contains("torch.load("));
